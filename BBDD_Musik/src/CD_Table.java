		...
		ListSelectionListener, KeyListener, MouseListener, ItemListener
{
	...
	private JMenuItem miNeu, miAendern, miLoeschen, miSchliessen, mi1, mi2, mi3;
	...
	private JRadioButton holdFilter;
	private String globalTypeMusic = "";
	
	
	
	private boolean showLineOutOfFilter = false;
	...
	private void initializeComponents()
	{
		...
		//2015.04.02 Alopez we introduce one radio botton
		holdFilter = new JRadioButton("Special HoldFilter");
		holdFilter.setBounds(860, 1, 150, 25);
		holdFilter.setFont(new Font("ARIAL", Font.CENTER_BASELINE, 10));
		holdFilter.addActionListener(this);
		this.add(holdFilter);
...
//		this.add(tbTypMusic);
		...
		mi1 = new JMenuItem("Update CD Information");
		mi1.addActionListener(this);

		mi2 = new JMenuItem("Show all the songs of this CD");
		mi2.addActionListener(this);
		
		mi3 = new JMenuItem("Delete only first song of this CD");
...
//	}
	...
	private void update_new_Line(long PrimaryKey)
	{
		...
		// if it is one update
		{
			...
			System.out.println("showLineOutOfFilter :" + showLineOutOfFilter);
	...
	private void DeleteOnlyFisrtSong(long PrimaryKey)
	{
		...
		int retValue = JOptionPane.showOptionDialog(this, " delete only firts Song",
				"Delete Only First Song of CD", JOptionPane.YES_NO_OPTION,
	...
	//2015.04.01 Alopez we delete all the CD not only the first song
	private void deleteEntry(String CD_Name)
	{
		...
		String SQL = "DELETE FROM cd_songs";
		SQL += " WHERE CD_Name = "+ insertQuaotes(CD_Name);

		int retValue = JOptionPane.showOptionDialog(this, 
				"<html>Do you want to delete the whole the CD : "+ CD_Name + "  and  all his songs?. <br> <br>"
				+ " (Möchten Sie die ganze CD löschen?)</html>",
				"All CD delete", JOptionPane.YES_NO_OPTION,
				JOptionPane.QUESTION_MESSAGE, null, options, options[1]);
	...
	public void actionPerformed(ActionEvent e)
	{
		if (e.getSource() == miSchliessen)
		  ...
		  { //deleteEntry(Long.parseLong(myTable.getValueAt(
			...
			deleteEntry(myTable.getValueAt(
				myTable.getSelectedRow(), 3).toString());
		  }
		...
		{
			//System.out.println("mi2  estoy dentro del Menu Item mi");
			DeleteOnlyFisrtSong(Long.parseLong(myTable.getValueAt(myTable.getSelectedRow(), 0).toString()));
			 
		}

		else if (e.getSource() == holdFilter)
		{
			//System.out.println("hallo holdFilter ");
			if (holdFilter.isSelected())
			{
				showLineOutOfFilter = true;
			}
				
			else 
			{
				showLineOutOfFilter = false;
			}
		}
	...
	public void mouseClicked(MouseEvent e)
	{
		...
		if (e.getClickCount() == 2)
			//update_new_Line(Long.parseLong(myTable.getValueAt(myTable.getSelectedRow(), 0).toString()));
			showAllSongsfromCD();
